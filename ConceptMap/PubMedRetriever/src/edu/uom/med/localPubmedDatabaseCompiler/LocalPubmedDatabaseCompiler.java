 ...
 * prohibited. PubMed provides all the published articles through ftp for mining researches. Thus, the entire database 
 * can be downloaded and saved locally. We present a Java Project that can compress the downloaded articles into a
 * binary file. 
 * 
 * The project consists of two tasks that can be used in combination or separately from the wrapper class:
 * LocalPubmedDatabaseCompiler.java. The two tasks are: 
 * 	(1) decompressing the zip files downloaded from PubMed ftp, and
