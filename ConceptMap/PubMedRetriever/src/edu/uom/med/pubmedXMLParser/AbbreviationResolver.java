...
public class AbbreviationResolver {
	...
	public ArrayList<String> resolveAbbreviation(String abstractLine) {
		...
		for(int i=0; i<sentences.size(); i++) { 
			...
			for(String sentTerm : sent) {
				Pattern escaper = Pattern.compile("([\\\\|*\\{\\}_])"); //for escaping \ | and *
				Matcher matcher = escaper.matcher(sentTerm);
				if(matcher.find()) sentTerm = matcher.replaceAll("").toString();
				
				Pattern p = Pattern.compile("[A-Z]");
				Matcher m = p.matcher(sentTerm);
				if(m.find()) abbreviation = sentTerm.trim();
				...
				if(abbreviation.contains("+")) { abbreviation = ""; }
				if(abbreviation.matches("(.+?)-(.+?)")) { abbreviation = ""; }
				if(abbreviation.matches("(.+?)\\((.+?)") || abbreviation.matches("(.+?)\\)(.+?)") ||
						abbreviation.matches("(.+?)\\)") || abbreviation.matches("\\((.+?)")) {
					abbreviation = "";
				}
	...
	/**
	 * Main method with an example sentence to test the performance of Abbreviation expansion algorithm
	 * 
	 * @param args
	 */
	
	public static void main(String[] args) {
		AbbreviationResolver ar = new AbbreviationResolver();
		
		String test = "This membrane protein is now called as by a new name alpha 1b glycoprotein (alpha1B). alpha1B is most important.";
		ArrayList<String> testResult = ar.resolveAbbreviation(test);
