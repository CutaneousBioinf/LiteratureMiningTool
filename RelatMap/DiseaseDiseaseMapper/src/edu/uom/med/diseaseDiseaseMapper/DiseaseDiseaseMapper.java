...
import edu.uom.med.conceptExtractor.DiseaseFinder;
 ...
 * project uses MedTagger from Mayo clinic for mapping. It filters sentences mapped with two diseases. 
 * 
 * 
 * @author Kalpana Raja
 *
...
public class DiseaseDiseaseMapper {
		...
		try {
			...
			for(String eachSentence : allSentences) {
				...
				if(arrLine1.length>1) {
					...
					if(!diseases.isEmpty()) {
						if(diseases.size()==2 && diseasesCUI_TUI.size()==2) {
							//skip abbreviations in the dictionary
							if(diseases.get(0).matches("^[A-Z0-9]") == true) continue;
							if(diseases.get(1).matches("^[A-Z0-9]") == true) continue;
									
							bw.append(eachSentence+"\t"+diseases.get(0)+"\t"+diseases.get(1)+"\t"+diseasesCUI_TUI.get(0)+"\t"+diseasesCUI_TUI.get(1));
							bw.append("\n");
						}	
					}
				}
